...
function fillEllipse(ellipsesList) {
  let c = 0;
  for (let e of ellipsesList) {
    e.fillColor = fillColors[c++ % fillColors.length];
  }
}

// Draw static white circles
...
class RingPattern {
  ...
  drawRegion(iR,oR,st,bg,pc) {
    noStroke(); fill(bg);
    this.drawDonut(iR,oR);
    if (st==='zigzag')   this.drawZigzagRing(iR,oR,72,pc);
    else if (st==='dots')this.drawDotsRing(iR,oR,6,pc);
    else if (st==='layered') this.drawLayeredRings(iR,oR,pc);
  }
  ...
  drawZigzagRing(iR,oR,steps,col) {
    ...
    for (let i=0;i<=steps;i++){
      let ang=TWO_PI*i/steps, r=(i%2?iR+off:oR-off);
  ...
  drawDotsRing(rMin,rMax, cnt, col){
    ...
    for(let i=0;i<cnt;i++){
      let r=lerp(rMin+m,rMax-m,i/(cnt-1)),
          base=floor(r), num=base+floor(random(-4,4));
      for(let j=0;j<num;j++){
...
function setup() {
  ...
  colorMode(HSB, 360, 100, 100);
   ...
   // Define ringConfigs using color()
const ringConfigs = [
    {
      x:  70, y:  70,
...
function draw() {
  ...
  ellipses.forEach(e => {
    ...
    // -- Draw circles: use noise for size and random HSB for stroke
noFill();  
strokeWeight(4);  
for (let i = 0; i < circles.length; i++) {  
  ...
  let d = map(n, 0, 1, circleBaseRadii[i] * 1, circleBaseRadii[i] * 5);  

  // White fill
  fill(0, 0, 100); 
  
  // Random stroke: hue 0–360, saturation 30–60, brightness 30–60
  let h = random(0, 360);  
  let s = random(30, 60);  
  let b = random(30, 60);  
  stroke(h, s, b);  
